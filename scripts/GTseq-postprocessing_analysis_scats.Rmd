...
```{r}
ScatLog <- read.csv(here("GTseq_September2024","Pv_scat_extracts_ComBay2018.csv"),header = TRUE)

Lib_geno <- read.csv(here("GTseq_September2024","PvSept2024Cor_Library_Genotypes.csv"), header=TRUE, row.names = NULL ,stringsAsFactors=FALSE,check.names=FALSE)

```

```{r}
Lib_counts <- read.csv(here("GTseq_September2024","PvSept2024Cor_Library_Counts.csv"), header = TRUE, row.names = NULL, check.names=FALSE)

ProbeSeq <- read.csv(here("GTseq_September2024","ProbeSeq.csv"),header=FALSE)

primerdecision <- read.csv(here("GTseq_September2024", "GTseqSep2024SeqTest_decsion.csv"), header = T, row.names = NULL, check.names = F)

metadata <- read.csv(here("GTseq_September2024","GTseq_Sep2024_metadata.csv"), header = T, check.names = F, row.names = NULL)
```

Do this once to remake the probeseq to remove the ones we are tossing
```{r}
primerdecision %>% 
  dplyr::rename("MarkerName" = "SNP_id") %>%
  dplyr::rename("Decision" = "Keep/Toss") %>%
  select(MarkerName, Decision) %>% 
  filter(Decision == "KEEP") -> FinalPrimers

ProbeSeq %>% 
  dplyr::rename("MarkerName" = "V1") -> ProbeSeq

left_join(FinalPrimers,ProbeSeq, by = "MarkerName") -> ProbeSeq2

ProbeSeq2 %>% select(!Decision) -> ProbeSeq2

write.csv(ProbeSeq2, "ProbeSeq_Sept2024_FinalSet.csv", row.names = FALSE, col.names = FALSE)
```
...
ProbeSeq %>% 
  ...
  mutate(Loc = "loc") %>% 
  mutate(num = 1:n()) %>% 
  mutate(num = str_pad(string = num, width = 3, side = 'left', pad = 0)) %>% 
  unite(Loci, c("Loc","num"), sep = "") %>% 
  mutate(Loci = case_when(MarkerName == "ZFX" ~ "LocZFX", 
                          MarkerName == "ZFY" ~ "LocZFY",
                          MarkerName == "SRY" ~ "LocSRY", TRUE ~ Loci))-> locimap
...
ScatLog %>% 
  mutate(SampleID = coalesce(SampleID, ScatID)) -> metadata
metadata %>%  dplyr::rename("Sample" = "SampleID") %>% select(ScatID,Sample,Collection_Location,day,month,year)-> metadata

left_join(metadata, Lib_metrics, by = "Sample") -> Lib_MD
left_join(Lib_MD, LibGenos, by = "Sample") -> Lib_geno_MD 

Lib_geno_MD %>% filter(!is.na(RawReads)) -> Lib_geno_MD

Lib_geno_MD %>%
  select(Sample, Collection_Location,day,month,year, RawReads, OTreads, PerON, PerGT, LocZFX,LocZFY, LocSRY) -> SexIDGeno
...
SexIDGeno %>%
  mutate(GTsex = case_when(LocZFX == "GG" & LocZFY == "AA" ~ "M",
                           LocZFX == "GG" & LocSRY == "TT" ~ "M",
                           LocZFX == "GG" & LocSRY == "0" ~ "F",
                           LocSRY == "TT" & LocZFX == "0"~ "M" ,
                           LocZFX == "0" & LocSRY == "0" ~ "U")) -> SexIDGeno
...
```{r}
Lib_counts %>% 
  select(!c("18OO0001.fastq", "%GT")) %>% 
  dplyr::rename("RawReads" = "Raw Reads", "OTreads" = "On-Target Reads", "PercOT" ="%On-Target") -> Lib_counts

Lib_counts %>%
  select(Sample, RawReads, OTreads, PercOT, ZFX, ZFY, SRY) -> SexIDcounts

left_join(metadata, SexIDcounts, by = "Sample") -> SexIDcounts

SexIDcounts %>%
  mutate(PropZY = (ZFY/ZFX)*100) %>%
  mutate(PropSY = (SRY/ZFX)*100) %>% 
  mutate(Scat_Sex = case_when(ZFX >= 1 & SRY == 0 ~ "F",
                          SRY >= 1 ~ "M",
                          ZFX == 0 & SRY == 0 ~ "Unk")) %>%
  mutate(YChromSex = case_when(SRY >= 1 ~ "M",
                               SRY == 0 & ZFX == 0 ~ "Fail",
                               SRY == 0 & ZFX < 10 ~ "Check",
                               SRY == 0 & ZFX >= 10 ~ "F")) %>%
  mutate(SexProptarget = case_when(PercOT < 10 & PropSY == 0 ~ "Check",
                                   PercOT > 10 & PropSY >= 1 ~ "M",
                                   PercOT > 10 & PropSY == 0 ~ "F",
                                   PercOT < 10 & PropSY == "NaN" ~ "Fail",
                                   PercOT < 10 & PropSY == "Inf" ~ "M",
                                   PercOT < 10 & PropSY >= 1 ~ "M",
                                   PercOT > 10 & PropSY <= 1 ~ "M-check"))    -> SexIDcounts2

```

Let's see which samples had 0% Genotyping aka failed outright
```{r}
SexIDGeno %>% filter(PerGT == 0) -> FailedSamples #that's 528 samples that had 0 GT% which is not good

```

Now let's pull out the ones that got more than 30 GT%
```{r}
SexIDGeno %>% filter(PerGT >= 30) -> SuccessSamples #61 scat samples that succeeded
```

So there are going to be a lot with no success but had reads so perhaps with better depth could get better results let's pull in the counts data
```{r}
left_join(SexIDGeno, Lib_counts, by= c("Sample", "RawReads", "OTreads")) -> SexGeno_allCounts
```
