...
genotypeDirectory="/home/erin/Projects/WDFW/SPSid/GTseq_September2024"
setwd(genotypeDirectory)
source("GTscore.R")

#SHOW GENOTYPING USING TEST DATASET FOR SPEED
singleSNP_locusTable<-read.delim("LocusTable_singleSNPs.txt",header=TRUE,check.names = F)
singleSNP_alleleReads<-read.delim("AlleleReads_singleSNPs.txt",header=TRUE,row.names=1,check.names = F)

haplotype_locusTable<-read.delim("LocusTable_haplotypes.txt",header=TRUE, check.names = F)
haplotype_alleleReads<-read.delim("AlleleReads_haplotypes.txt",header=TRUE,row.names=1,check.names = F)
...
polyGenResults_singleSNP_adjusted <- polyGen(singleSNP_locusTable,singleSNP_alleleReads_adjusted)
write.table(polyGenResults_singleSNP_adjusted,"polyGenResults_singleSNP_adjusted.txt",quote=FALSE,sep="\t")
...
sampleMetaData<-read.delim(here("sample_metadata_rubias.txt"),header=TRUE)
...
exportRubias(polyGenResults_singleSNP_adjusted,singleSNP_locusTable,sampleMetaData,filename="polyGenResults_singleSNP_adjusted_rubias.txt")
...
#summarize single SNP adjusted results
singleSNP_adjusted_summary<-summarizeGTscore(singleSNP_alleleReads_adjusted,singleSNP_locusTable, polyGenResults_singleSNP_adjusted)
#view results
head(singleSNP_adjusted_summary)
#write results
write.table(singleSNP_adjusted_summary,"singleSNP_adjusted_summary.txt",quote=FALSE,sep="\t",row.names=FALSE)
