...
SNPstats <- read.table(here("Seal_ID_SNP_panel","world_beagle_SNP_stats"), header = T)
...
PacStats <- read.table(here("Seal_ID_SNP_panel","pacific_beagle_SNP_stats"), header = F)
...
Alright so now we know where all the SNPs in the Pac Seal set are that we may want to develop primers for. We will use these later to compare to the ones nathan found 
...
SNPsubset %>%
  subset(refseq %in% markers2V) -> SNPsubset2 # 2776 SNPs left ATTENTION: we use this later
...
MicrohapList %>%
  unite("PrimerID",refseq,POS, sep="_") -> MicrohapNamelist

write.csv(MicrohapNamelist, "MicroHapNameList.csv", row.names = FALSE)
```
